...
fn build_menu(app: &AppHandle, lang: &str) -> tauri::Result<Menu<tauri::Wry>> {
    ...
    let time_image_interval_secs = prefs
        .as_ref()
        .map(|prefs| prefs.time_image_interval_secs)
        .unwrap_or(crate::prefs::DEFAULT_TIME_IMAGE_INTERVAL_SECS);
    ...
    for secs in [0_u16, 60, 120, 180, 300] {
        let label = if time_image_interval_secs == secs {
            format!("✓ {}", crate::time_image_interval_label(secs, lang))
        } else {
            crate::time_image_interval_label(secs, lang)
        };
        time_image_items.push(MenuItem::with_id(
            app,
            format!("time-image-interval-{secs}"),
            label,
            true,
            None::<&str>,
        )?);
    ...
    let time_image_refs: Vec<&dyn tauri::menu::IsMenuItem<tauri::Wry>> = time_image_items
        ...
        .collect();
    let time_image_sub =
        Submenu::with_items(app, t("timeImageInterval", lang), true, &time_image_refs)?;
...
fn handle_tray_event(app: &AppHandle, id: &str) {
    match id {
        ...
        | "time-image-interval-300" => {
            let secs = id
                .strip_prefix("time-image-interval-")
                .and_then(|value| value.parse::<u16>().ok())
                .unwrap_or(crate::prefs::DEFAULT_TIME_IMAGE_INTERVAL_SECS);
            crate::set_time_image_interval_secs(app, secs);
            rebuild_current_menu(app);
        }
        "lang-en" | "lang-zh" => {
