<script lang="ts">
  ...
  function timeImageIntervalLabel(seconds: number): string {
    if (!data) return `${seconds}s`;
    if (seconds === 0) return t('off');
    const minutes = seconds / 60;
    if (data.lang === 'zh') return `${minutes} 分钟内随机`;
    return minutes === 1 ? 'Random within 1 minute' : `Random within ${minutes} minutes`;
  }
...
<div class="menu">
  ...
  <div class="item has-sub" role="button" tabindex="-1" onmouseenter={() => activeSubmenu = 'time-image'} onmouseleave={() => activeSubmenu = null}>
    <span>{t('timeImageInterval')}</span>
    <span class="value">{timeImageIntervalLabel(data.time_image_interval_secs)}</span>
    <span class="arrow">›</span>
    {#if activeSubmenu === 'time-image'}
      <div class="submenu">
        {#each [0, 60, 120, 180, 300] as seconds}
          <button class="item" class:checked={data.time_image_interval_secs === seconds} onclick={() => action(`ctx-time-image-interval-${seconds}`)}>{timeImageIntervalLabel(seconds)}</button>
        {/each}
      </div>
    {/if}
  </div>
