...
pub(crate) fn time_image_interval_label(secs: u16, lang: &str) -> String {
    if secs == 0 {
        return i18n::t("off", lang);
    }
    let minutes = secs / 60;
    if lang == "zh" {
        format!("{minutes} 分钟内随机")
    } else if minutes == 1 {
        "Random within 1 minute".into()
    } else {
        format!("Random within {minutes} minutes")
    }
}
...
pub(crate) fn set_time_image_interval_secs(app: &AppHandle, secs: u16) {
    let Some(prefs_state) = app.try_state::<SharedPrefs>() else {
        return;
    };
    let mut prefs = prefs_state.lock_or_recover();
...
fn logical_drag_position(
    ...
    (
        (base_x + dx as f64).round() as i32,
        (base_y + dy as f64).round() as i32,
    )
}
    ...
    if let Some(pet) = app.get_webview_window("pet") {
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            if let Ok(pos) = pet.outer_position() {
                d.start_win_x = pos.x as f64 / scale_factor;
                d.start_win_y = pos.y as f64 / scale_factor;
            }
        }
        ...
        #[cfg(not(target_os = "macos"))]
        {
            ...
            if let Ok(pos) = pet.outer_position() {
                d.start_win_x = pos.x as f64;
                d.start_win_y = pos.y as f64;
            }
        }
    } else {
...
fn drag_move(app: AppHandle, drag: tauri::State<SharedDrag>, x: f64, y: f64) {
    ...
    #[cfg(target_os = "macos")]
    let current = DragPoint { x, y };
    #[cfg(not(target_os = "macos"))]
    let current = logical_to_physical(DragPoint { x, y }, drag_scale_factor);
    let start = DragPoint { x: smx, y: smy };

    // Don't start moving until the mouse has moved past the drag threshold
    if !dragging {
        let distance = drag_distance(start, current);
        #[cfg(target_os = "macos")]
        let distance = distance * drag_scale_factor;
    ...
    #[cfg(target_os = "macos")]
    let (new_x, new_y) = logical_drag_position(base_x, base_y, start, current);
    #[cfg(not(target_os = "macos"))]
    ...
    #[cfg(target_os = "macos")]
    let pointer_point = DragPoint { x, y };
    #[cfg(not(target_os = "macos"))]
    let pointer_point = current;

    let pointer_monitor = app
        .monitor_from_point(pointer_point.x, pointer_point.y)
        .ok()
        .flatten();

    #[cfg(not(target_os = "macos"))]
    if let Some(monitor) = pointer_monitor
        .map(|monitor| windows::monitor_area(&monitor))
        .or_else(|| {
            ...
            windows::monitor_for_bounds(&app, &probe_bounds)
                .or_else(|| windows::current_monitor_for_pet(&app))
        })
    {
        let min_visible = (30.0 * drag_scale_factor).round() as i32;
        (new_x, new_y) =
    ...
    if let Some(pet) = app.get_webview_window("pet") {
        #[cfg(target_os = "macos")]
        let _ = pet.set_position(LogicalPosition::new(new_x as f64, new_y as f64));
        #[cfg(not(target_os = "macos"))]
    ...
    #[cfg(target_os = "macos")]
    let (new_x, new_y) = {
        let scale = pointer_monitor
            .as_ref()
            .map(|monitor| monitor.scale_factor())
            .unwrap_or(drag_scale_factor);
        (
    ...
    // During macOS mixed-DPI drags, the pet window is moved in logical coords.
    // The hit window still uses the normal physical sync path, so moving it
    // every frame can introduce a second, stale coordinate basis.
    #[cfg(not(target_os = "macos"))]
    {
...
fn drag_end(
    ...
    if is_mini {
        if !was_dragging {
            // Click in mini mode — exit to normal idle
            cancel_pending_task(&abort_handle);
            mini::do_exit_mini(&app);
            transition(&app, &state, "idle", "clyde-idle-follow.svg");
        ...
        if was_dragging {
            ...
            // Click (not drag) in normal mode — return to idle
            cancel_pending_task(&abort_handle);
            transition(&app, &state, "idle", "clyde-idle-follow.svg");
        }
    }
...
fn show_context_menu(
    ...
    let (
        ...
        auto_dnd_meetings,
        autostart,
        permission_decision_window_secs,
        time_image_interval_secs,
    ) = {
    ...
    let mut time_image_items = Vec::new();
    for secs in [0_u16, 60, 120, 180, 300] {
        let label = if time_image_interval_secs == secs {
            format!("✓ {}", time_image_interval_label(secs, &lang))
        } else {
            time_image_interval_label(secs, &lang)
        };
        if let Ok(item) = MenuItem::with_id(
            &app,
            format!("ctx-time-image-interval-{secs}"),
            label,
            true,
            None::<&str>,
        ) {
            time_image_items.push(item);
        }
    }
    let time_image_refs: Vec<&dyn tauri::menu::IsMenuItem<tauri::Wry>> = time_image_items
        .iter()
        .map(|item| item as &dyn tauri::menu::IsMenuItem<tauri::Wry>)
        .collect();
    if let Ok(sub) = Submenu::with_items(
        &app,
        i18n::t("timeImageInterval", &lang),
        true,
        &time_image_refs,
    ) {
        items.push(Box::new(sub));
...
pub(crate) fn do_toggle_visibility(app: &AppHandle) {
    let hidden = app
        .try_state::<HiddenFlag>()
        .map(|f| *f.lock_or_recover())
        .unwrap_or(false);
    if hidden {
        do_show_from_tray(app);
    } else {
        do_hide_to_tray(app);
    }
}
...
#[tauri::command]
fn finalize_bubble_close(app: AppHandle, bubbles: tauri::State<permission::BubbleMap>, id: String) {
...
fn handle_context_menu_event(app: &AppHandle, state: &SharedState, id: &str) {
    ...
    match action {
        ...
        "time-image-interval-0" => {
            set_time_image_interval_secs(app, 0);
            refresh_tray = true;
        }
        "time-image-interval-60" => {
            set_time_image_interval_secs(app, 60);
            refresh_tray = true;
        }
        "time-image-interval-120" => {
            set_time_image_interval_secs(app, 120);
            refresh_tray = true;
        }
        "time-image-interval-180" => {
            set_time_image_interval_secs(app, 180);
            refresh_tray = true;
        }
        "time-image-interval-300" => {
            set_time_image_interval_secs(app, 300);
            refresh_tray = true;
        }
...
fn setup_active_space_observer(app: &AppHandle, bubbles: permission::BubbleMap) {
    ...
    macos_spaces::install_active_space_observer(move || {
        ...
        let _ = app_for_space.run_on_main_thread(move || {
            ...
            let bubble_ids: Vec<String> =
                bubbles_for_main.lock_or_recover().keys().cloned().collect();
...
#[cfg(target_os = "macos")]
fn setup_codex_activation_observer(
    app: &AppHandle,
    state: SharedState,
    bubbles: permission::BubbleMap,
) {
    let app_for_codex = app.clone();
    macos_spaces::install_codex_activation_observer(move || {
        dismiss_transient_ui(&app_for_codex, &state, &bubbles);
    });
}
        ...
        .setup(move |app| {
            ...
            *shared_prefs.lock_or_recover() = prefs.clone();
            ...
            #[cfg(target_os = "macos")]
            {
                ...
                setup_codex_activation_observer(
                    app.handle(),
                    shared_state.clone(),
                    bubble_map.clone(),
                );
